...
## gRPC rate-limit surface (Trifecta Step 3)

Alongside the servlet filter, fluxguard exposes a synchronous gRPC service —
`fluxguard.ratelimit.v1.RateLimit` on **`:9099`** — that another service (bankops-portal)
calls *before committing*, mirroring the fluxa fraud-eval integration. The gRPC path reuses
the same `RateLimitEngine` decision core (token bucket / sliding window, Redis Lua, fail-open)
as the filter — one circuit breaker, one `rate_limit.decision` span. gRPC health + reflection
are enabled, and a `GrpcTelemetry` server interceptor joins each request to the shared Jaeger
trace. See ADR-007 / ADR-008 and [`docs/specs/2026-06-02-bankops-ratelimit-grpc-integration.md`](docs/specs/2026-06-02-bankops-ratelimit-grpc-integration.md).

- **`CheckLimit(policy, subject, client_ip, idempotency_key)`** — called pre-commit.
  `POLICY_TRANSACTION` / `POLICY_OPS_RELEASE` / `POLICY_OPS_REJECT` use a per-subject token
  bucket; a replayed `idempotency_key` returns the prior decision without spending a token.
  `POLICY_LOGIN` reads an IP-keyed, failures-only sliding window. `DECISION_DENY` → the caller
  maps `429` + `Retry-After` from `retry_after_ms`; `fail_open=true` marks a Redis-down allow.
- **`ReportLoginFailure(subject, client_ip)`** — called after a failed auth; increments the
  login failure window (keyed on `client_ip`, never username → no account-lockout DoS).

```bash
# List the service (reflection enabled)
grpcurl -plaintext localhost:9099 list

# A transaction check
grpcurl -plaintext \
  -d '{"policy":"POLICY_TRANSACTION","subject":"acct-42","idempotency_key":"txn-1"}' \
  localhost:9099 fluxguard.ratelimit.v1.RateLimit/CheckLimit
```
...
### gRPC surface (2026-06-02 — live local run)

| Scenario          | Result                                                                 |
|-------------------|------------------------------------------------------------------------|
| transaction burst | drain→deny→recover: 21 allowed / 379 denied / 10 recover after refill, p95 19ms |
| login brute-force | 5 allowed → 25 denied once the IP failure window fills, p95 3ms          |

Run via [`k6/scripts/run-grpc-benchmark.sh`](k6/scripts/run-grpc-benchmark.sh) (transaction burst) and [`k6/scripts/grpc-login-bruteforce.js`](k6/scripts/grpc-login-bruteforce.js) (login brute-force) against a live server.
...
| [ADR-007](docs/adr/ADR-007-grpc-ratelimit-surface.md) | Synchronous gRPC rate-limit surface     | Accepted |
| [ADR-008](docs/adr/ADR-008-outcome-aware-login-limiting.md) | Outcome-aware login rate limiting | Accepted |
